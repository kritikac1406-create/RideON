...
const newCardTemplate = `
      ...
      data.forEach(car => {
          container.innerHTML += \`
          <div class="car-card bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 flex flex-col group relative transition-all duration-300 hover:-translate-y-1 hover:shadow-xl p-4 min-w-0 w-full"
               data-name="\${car.name || ''}" 
             ...
             <!-- Image Box (Pinned to Bottom to Eliminate Floating Vertical Gaps) -->
             <div class="w-full relative overflow-hidden group-hover:opacity-95 transition-opacity mb-4 bg-transparent border-none flex items-end justify-center" style="height: 176px;">
                <img src="\${car.image_url}" class="w-full h-full object-contain relative z-10 transform group-hover:scale-105 transition-transform duration-500 ease-out p-1" style="object-position: bottom;" onerror="this.src='https://via.placeholder.com/400x200?text=No+Image'">
             </div>
             ...
             <div class="flex flex-wrap items-center gap-2 mb-5">
                <!-- Fuel -->
                <div class="flex items-center gap-1.5 text-gray-700 bg-gray-50 px-2 py-1 rounded border border-gray-100 min-w-0 max-w-full overflow-hidden">
                   ...
                   <span class="font-bold uppercase tracking-wider truncate" style="font-size: 10px;">\${car.fuel_type || 'Petrol'}</span>
                </div>
                ...
                <div class="flex items-center gap-1.5 text-gray-700 bg-gray-50 px-2 py-1 rounded border border-gray-100 min-w-0 max-w-full overflow-hidden">
                   ...
                   <span class="font-bold uppercase tracking-wider truncate" style="font-size: 10px;">\${car.transmission || 'Auto'}</span>
                </div>
             </div>
             ...
             <div class="flex justify-between items-center mt-auto flex-wrap gap-2">
                <div class="flex flex-col flex-grow min-w-0">
                   <p class="text-2xl font-black leading-none truncate" style="color: #114b72;">
                ...
                <button onclick="initiateBooking('\${car.id}', '\${car.name.replace(/'/g, "&apos;")}', '\${car.rent_per_hour}', '\${car.image_url}')" 
                   class="px-6 py-2.5 rounded-lg font-black uppercase tracking-widest transition-all text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 flex-shrink-0" 
...
targetFiles.forEach(f => {
    ...
    if (content.includes(startAnchor) && content.includes(endAnchor)) {
        ...
        console.log("Deployed V11 Stabilized Grid Patch to " + f);
    }
});
