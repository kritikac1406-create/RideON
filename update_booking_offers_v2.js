...
// 1. Regex to replace loadCar
const loadCarRegex = /async function loadCar\(\) \{[\s\S]*?document\.getElementById\("plateWrap"\)\.innerHTML = `[^`]+`;\s*\}/m;
const newLoadCar = `async function loadCar() {
...
content = content.replace(loadCarRegex, newLoadCar);
// Strip out duplicate loadCar() calls that may arise from regex overlapping the original call
content = content.replace(/loadCar\(\);\s*loadCar\(\);/g, 'loadCar();');

// 2. Regex to replace calculateTotal
const calcRegex = /function calculateTotal\(\) \{[\s\S]*?const hours = Math\.ceil\(\(end - start\) \/ \(1000 \* 60 \* 60\)\);[\s\S]*?document\.getElementById\("totalAmount"\)\.textContent = `₹\$\{hours \* carData\.rent_per_hour\}`;\s*\}/m;
const newCalc = `function calculateTotal() {
  ...
  if (activeOffer) {
      ...
      if (discountPercent > 0) {
          ...
          badge.className = "text-green-700 font-bold uppercase tracking-widest text-[10px] mb-2 mt-1 flex items-center justify-center gap-1.5 bg-green-100/80 px-3 py-1.5 rounded-lg border border-green-200 shadow-sm w-max transform transition-all";
...
console.log("Deployed Force-Regex Discount Logic.");
