      ...
      data.forEach(car => {
          ...
          <div class="car-card bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 flex flex-col group relative transition-all duration-300 hover:-translate-y-1 hover:shadow-xl p-4"
             ...
             <div class="flex flex-wrap items-center gap-3 mb-5">
                ...
                <!-- KM -->
                <div class="flex items-center gap-1.5 text-gray-700 bg-gray-50 px-2 py-1 rounded border border-gray-100">
                   <div class="w-6 h-6 rounded-full bg-green-50 flex items-center justify-center border border-green-100 text-green-600 shadow-sm">
                      <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                   </div>
                   <span class="font-bold uppercase tracking-wider" style="font-size: 10px;">Unltd KM</span>
                </div>
             </div>
             ...
             <div class="flex justify-between items-center mt-auto">
                <div class="flex flex-col">
                   <p class="font-bold text-gray-400 uppercase tracking-widest mb-1 flex items-center gap-1" style="font-size: 9px;"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> Unlimited KM</p>
...
const updatedFilterScript = `function applyFilters() {
...
targetFiles.forEach(f => {
    ...
    // 2. Inject the robust Filter Algorithm
    const filterStart = /function applyFilters\(\)[\s\S]*?window\.applyFilters\s*=\s*applyFilters;/m;
    if (content.match(filterStart)) {
        content = content.replace(filterStart, updatedFilterScript);
    }

    fs.writeFileSync(f, content, 'utf8');
    console.log("Deployed V8 Bugfix Patches to " + f);
});
